...
def main():
    ...
    ap.add_argument("--seeds", default="42,1337,2025")
    ...
    print("="*92)
    print("🦷 HARD v2 PROGRESSIVE TRAINING (multi-seed)")
    ...
    for seed in seeds:
        ...
        for stage_i, (img_size, E, bs, lr) in enumerate(zip(sizes, epochs, bss, lrs), start=1):
            ...
            # If not first stage and we have a previous best, load it to continue
            ...
            best_val = float("inf")
            stage_tag = f"{args.run_name}_seed{seed}_stage{stage_i}_{img_size}"  # Fixed: run_name not run-name
            save_path = os.path.join(args.save_dir, f"{stage_tag}.pt")

            for epoch in range(1, E + 1):
                train_loss = train_one_epoch(model, dl_train, optimizer, scaler, device, criterion, epoch, E)
            ...
            prev_ckpt_path = save_path  # carry best from this stage to next
